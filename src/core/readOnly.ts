 ...
 * @returns 返回一个只读的代理数据
 */
export const readOnly = <T extends TAnyObj>(data: T, mode: TMode<T> = 'default'): T => {
	return new Proxy(data, {
		get(target: T, p: keyof T) {
			...
			if (type === 'function') {
				if (mode === 'default') return readOnly(target[p], mode)
...
export const isReadOnly = <T extends TAnyObj>(target: T) => {
	...
	if (!['object', 'array', 'function'].includes(type)) return false
